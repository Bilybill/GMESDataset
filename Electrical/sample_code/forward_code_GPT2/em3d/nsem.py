...
class Data:
    ...
    def get_complex_series(self, location_xy, orientation='xy', rx_type='impedance'):
        freqs, vals = self._collect_complex_series(location_xy, orientation=orientation, rx_type=rx_type)
        return freqs, vals

    def get_apparent_resistivity_phase(self, location_xy, orientation='xy'):
        freqs, z = self._collect_complex_series(location_xy, orientation=orientation, rx_type='impedance')
        if freqs.size == 0:
            return freqs, np.array([]), np.array([])
        rho = (np.abs(z) ** 2) / (4e-7 * math.pi * 2.0 * math.pi * freqs)
        phase = np.angle(z, deg=True)
        return freqs, rho, phase
...
class Simulation3DPrimarySecondary:
    def __init__(self, mesh: Grid3D, survey: Survey, sigma: torch.Tensor, sigmaPrimary: torch.Tensor | None = None,
                 ...
                 use_block_secondary: bool = True, precond: str = 'flex', layered_bg_mode: str = 'mean'):
        self.mesh = mesh
        self.survey = survey
        self.sigma = sigma
