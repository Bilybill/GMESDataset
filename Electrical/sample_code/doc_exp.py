...
def run(plotIt=True):
    ...
    if plotIt:
        # Plot model slices and survey layout
        fig_model = plt.figure(figsize=(12, 5))

        # Model slice (X-normal)
        ax1 = fig_model.add_subplot(121)
        collect_obj = M.plot_slice(np.log10(sig), grid=True, normal="X", ax=ax1)[0]
        plt.colorbar(collect_obj, ax=ax1, label="log10(Conductivity)")
        ax1.set_title("Model Slice (X-normal)")
        ax1.set_xlabel("Y (m)")
        ax1.set_ylabel("Z (m)")

        # Survey Layout & Model Slice (Z-normal)
        ax2 = fig_model.add_subplot(122)
        collect_obj2 = M.plot_slice(np.log10(sig), grid=True, normal="Z", ax=ax2)[0]
        plt.colorbar(collect_obj2, ax=ax2, label="log10(Conductivity)")

        # Overlay receivers
        ax2.scatter(rx_loc[:, 0], rx_loc[:, 1], c='r', s=10, label='Receivers')
        ax2.set_title("Survey Layout (Z-normal)")
        ax2.set_xlabel("X (m)")
        ax2.set_ylabel("Y (m)")
        ax2.legend()
        plt.tight_layout()
