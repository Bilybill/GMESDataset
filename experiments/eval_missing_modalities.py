...
def _evaluate_with_missing_modalities(model, loader, device, drop_modalities: tuple[str, ...]):
    model.eval()
    logits_list = []
    labels_list = []
    for batch in loader:
        modalities = {
            ...
            if name not in drop_modalities
        }
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Evaluate missing-modality robustness for GMES-3D classifiers.")
    ...
    parser.add_argument("--drop-modalities", type=str, nargs="+", required=True)
...
def main():
    ...
    drop_modalities = normalize_modalities(args.drop_modalities)
    ...
    with torch.no_grad():
        metrics = _evaluate_with_missing_modalities(model, loader, device, drop_modalities=drop_modalities)
    print(json.dumps({"split": args.split, "drop_modalities": list(drop_modalities), "metrics": metrics}, ensure_ascii=True, indent=2))
